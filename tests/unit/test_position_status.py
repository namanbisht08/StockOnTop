...
def test_entry_line_shows_day_and_date():
    entry = _priced_entry(entry_date=date(2026, 8, 24))
    message = render_daily_status_message([entry])

    assert "Entry: ₹100.00 (Monday, 24 Aug 2026)" in message
    # Invested/Current value stay plain - no formula annotation
    assert "Invested: ₹1,000.00\n" in message
    assert "Current value: ₹1,200.00\n" in message
